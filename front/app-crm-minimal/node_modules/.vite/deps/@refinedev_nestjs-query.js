import {
  ...
  parse,
  print
} from "./chunk-HYS4OCXC.js";
import {
  require_browser_ponyfill
} from "./chunk-G42DJUAS.js";
import {
  require_Symbol,
  require_baseGetTag,
  require_isObject,
  require_isSymbol,
  require_root
} from "./chunk-TMGIKD35.js";
import {
  require_pluralize
} from "./chunk-5QKMVZBF.js";
import {
  __commonJS,
  __toESM
...
// node_modules/@refinedev/nestjs-query/dist/esm/index.js
var _ = { eq: "eq", ne: "neq", lt: "lt", gt: "gt", lte: "lte", gte: "gte", in: "in", nin: "notIn" };
var E = (r, e) => {
  if (r === "contains")
    return { iLike: `%${e}%` };
  if (r === "ncontains")
  ...
  if (r === "between") {
    ...
    return e.length !== 2 ? {} : { between: { lower: e[0], upper: e[1] } };
  }
  return { [_[r]]: e };
};
var y = (r) => {
  let e = {};
  return r.filter((t) => Array.isArray(t.value) && t.value.length === 0 ? false : !!t.value).map((t) => t.operator === "and" || t.operator === "or" ? (0, import_set.default)(e, t.operator, [y(t.value)]) : "field" in t ? (0, import_set.default)(e, t.field, E(t.operator, t.value)) : {}), e;
};
var $ = (r) => r.map((e) => ({ field: e.field, direction: e.order.toUpperCase() }));
var L = (r) => {
  if (r.mode === "off")
    return { limit: 2147483647 };
  if (r.mode === "server" && !(!r.current || !r.pageSize))
    return { limit: r.pageSize, offset: (r.current - 1) * r.pageSize };
};
var O = ({ resource: r, filters: e, meta: t }) => {
  let i = `created${(0, import_camelcase2.default)((0, import_pluralize2.singular)(r), { pascalCase: true })}`, s = {};
  e && (s.input = { type: (0, import_camelcase2.default)(`create_${(0, import_pluralize2.singular)(r)}_subscription_filter_input`, { pascalCase: true }), required: true, value: { filter: y(e.filter((o) => !o.field.includes("."))) } });
  let { query: n, variables: a } = v.subscription({ operation: i, fields: t.fields, variables: s });
  return { query: n, variables: a, operation: i };
};
var x = ({ resource: r, filters: e, meta: t }) => {
  let i = `updatedOne${(0, import_camelcase2.default)((0, import_pluralize2.singular)(r), { pascalCase: true })}`, s = {};
  e && (s.input = { type: (0, import_camelcase2.default)(`update_one_${(0, import_pluralize2.singular)(r)}_subscription_filter_input`, { pascalCase: true }), required: true, value: { filter: y(e.filter((o) => !o.field.includes("."))) } });
  let { query: n, variables: a } = v.subscription({ operation: i, fields: t.fields, variables: s });
  return { query: n, variables: a, operation: i };
};
var B = ({ resource: r, filters: e, meta: t }) => {
  let i = `deletedOne${(0, import_camelcase2.default)((0, import_pluralize2.singular)(r), { pascalCase: true })}`, s = {};
  e && (s.input = { type: (0, import_camelcase2.default)(`delete_one_${(0, import_pluralize2.singular)(r)}_subscription_filter_input`, { pascalCase: true }), required: true, value: { filter: y(e.filter((o) => !o.field.includes("."))) } });
  let { query: n, variables: a } = v.subscription({ operation: i, fields: t.fields.filter((o) => typeof o != "object"), variables: s });
  return { query: n, variables: a, operation: i };
};
var S = async (r, { resource: e, ids: t, meta: i }) => {
  let s = (0, import_camelcase.default)(e), { query: n, variables: a } = l.query({ operation: s, fields: [{ nodes: (i == null ? void 0 : i.fields) || ["id"] }], variables: { filter: { type: (0, import_camelcase.default)(`${(0, import_pluralize.singular)(e)}Filter`, { pascalCase: true }), required: true, value: { id: { in: t } } } } });
  return { data: (await r.request(n, a))[s].nodes };
};
var P = (r) => ({ getList: async ({ resource: e, pagination: t, sorters: i, filters: s, meta: n }) => {
  let a = (0, import_camelcase.default)(e), o = L(t || {}), f = {};
  s && (f.filter = { type: (0, import_camelcase.default)(`${(0, import_pluralize.singular)(e)}Filter`, { pascalCase: true }), required: true, value: y(s) }), i && (f.sorting = { type: (0, import_camelcase.default)(`${(0, import_pluralize.singular)(e)}Sort`, { pascalCase: true }), required: true, list: [true], value: $(i) }), o && (f.paging = { type: "OffsetPaging", required: true, value: o });
  let { query: c, variables: g } = l.query({ operation: a, fields: [{ nodes: n == null ? void 0 : n.fields }, "totalCount"], variables: f }), d = await r.request(c, g);
  return { data: d[a].nodes, total: d[a].totalCount };
}, getMany: async ({ resource: e, ids: t, meta: i }) => await S(r, { resource: e, ids: t, meta: i }), create: async ({ resource: e, variables: t, meta: i }) => {
  let s = `createOne${(0, import_camelcase.default)((0, import_pluralize.singular)(e), { pascalCase: true })}`, { query: n, variables: a } = l.mutation({ operation: s, fields: (i == null ? void 0 : i.fields) || ["id"], variables: { input: { type: `CreateOne${(0, import_camelcase.default)((0, import_pluralize.singular)(e), { pascalCase: true })}Input`, required: true, value: { [(0, import_camelcase.default)((0, import_pluralize.singular)(e))]: t } } } });
  return { data: (await r.request(n, a))[s] };
}, createMany: async ({ resource: e, variables: t, meta: i }) => {
  let s = `createMany${(0, import_camelcase.default)(e, { pascalCase: true })}`, { query: n, variables: a } = l.mutation({ operation: s, fields: (i == null ? void 0 : i.fields) || ["id"], variables: { input: { type: `CreateMany${(0, import_camelcase.default)(e, { pascalCase: true })}Input`, required: true, value: { [(0, import_camelcase.default)(e)]: t } } } });
  return { data: (await r.request(n, a))[s] };
}, update: async ({ resource: e, id: t, variables: i, meta: s }) => {
  let n = `updateOne${(0, import_camelcase.default)((0, import_pluralize.singular)(e), { pascalCase: true })}`, { query: a, variables: o } = l.mutation({ operation: n, fields: (s == null ? void 0 : s.fields) || ["id"], variables: { input: { type: `UpdateOne${(0, import_camelcase.default)((0, import_pluralize.singular)(e), { pascalCase: true })}Input`, required: true, value: { id: t, update: i } } } });
  return { data: (await r.request(a, o))[n] };
}, updateMany: async ({ resource: e, ids: t, variables: i, meta: s }) => {
  let n = `updateMany${(0, import_camelcase.default)(e, { pascalCase: true })}`, { query: a, variables: o } = l.mutation({ operation: n, fields: ["updatedCount"], variables: { input: { type: `UpdateMany${(0, import_camelcase.default)(e, { pascalCase: true })}Input`, required: true, value: { filter: { id: { in: t } }, update: i } } } });
  return await r.request(a, o), await S(r, { resource: e, ids: t, meta: s });
}, getOne: async ({ resource: e, id: t, meta: i }) => {
  let s = (0, import_camelcase.default)((0, import_pluralize.singular)(e)), { query: n, variables: a } = l.query({ operation: s, fields: (i == null ? void 0 : i.fields) || ["id"], variables: { id: { type: "ID", required: true, value: t } } });
  return { data: (await r.request(n, a))[s] };
}, deleteOne: async ({ resource: e, id: t, meta: i }) => {
  let s = `deleteOne${(0, import_camelcase.default)((0, import_pluralize.singular)(e), { pascalCase: true })}`, { query: n, variables: a } = l.mutation({ operation: s, fields: (i == null ? void 0 : i.fields) || ["id"], variables: { input: { type: `DeleteOne${(0, import_camelcase.default)((0, import_pluralize.singular)(e), { pascalCase: true })}Input`, required: true, value: { id: t } } } });
  return { data: (await r.request(n, a))[s] };
}, deleteMany: async ({ resource: e, ids: t, meta: i }) => {
  let s = `deleteMany${(0, import_camelcase.default)(e, { pascalCase: true })}`, { query: n, variables: a } = l.mutation({ operation: s, fields: ["deletedCount"], variables: { input: { type: `DeleteMany${(0, import_camelcase.default)(e, { pascalCase: true })}Input`, required: true, value: { filter: { id: { in: t } } } } } });
  return await r.request(n, a), { data: [] };
}, getApiUrl: () => {
  throw Error("Not implemented on refine-nestjs-query data provider.");
}, custom: async ({ url: e, method: t, headers: i, meta: s }) => {
  if (e && r.setEndpoint(e), r.setHeaders(i), s != null && s.rawQuery)
    return { data: await r.request(s.rawQuery, s.variables) };
  if (s)
    if (s.operation)
      if (t === "get") {
        let { query: n, variables: a } = l.query({ operation: s.operation, fields: s.fields, variables: s.variables });
        return { data: (await r.request(n, a))[s.operation] };
      } else {
        let { query: n, variables: a } = l.mutation({ operation: s.operation, fields: s.fields, variables: s.variables });
        return { data: (await r.request(n, a))[s.operation] };
      }
    else
...
var M = P;
var C = (r, { callback: e, params: t }, i) => {
  let s = { created: O, updated: x, deleted: B }, { resource: n, meta: a, filters: o, subscriptionType: f, id: c, ids: g } = t ?? {}, d = s[i], { operation: w, query: h, variables: m } = d({ ids: g, id: c, resource: n, filters: o, meta: a, subscriptionType: f }), F = (V) => {
    e(V.data[w]);
  };
  return r.subscribe({ query: h, variables: m }, { next: F, error: console.error, complete: () => null });
};
var N = (r) => ({ subscribe({ callback: e, params: t }) {
  let { resource: i, meta: s, filters: n, subscriptionType: a, id: o, ids: f } = t ?? {};
  if (!s)
  ...
  let c = [];
  if ((t == null ? void 0 : t.subscriptionType) === "useList") {
    let d = C(r, { callback: e, params: t }, "created"), w = C(r, { callback: e, params: t }, "updated"), h = C(r, { callback: e, params: t }, "deleted");
    c.push(d), c.push(w), c.push(h);
  }
  if ((t == null ? void 0 : t.subscriptionType) === "useOne") {
    let d = C(r, { callback: e, params: t }, "updated");
    c.push(d);
  }
  return () => {
    c.forEach((d) => d());
